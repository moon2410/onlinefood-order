...
if (isset($_GET['id'])) {
    ...
    // Fetch order details
    $sql = "SELECT * FROM tbl_ad_order WHERE id='$order_id'";
    $res = mysqli_query($conn, $sql);
    $order = mysqli_fetch_assoc($res);

    // Fetch items for the order
    $sql_items = "SELECT * FROM tbl_ad_order_items WHERE order_id='$order_id'";
    ...
    while ($row = mysqli_fetch_assoc($res_items)) {
        ...
        $items[] = [
            ...
            'quantity' => $quantity,
            'price' => $row['price'],
            'total' => $quantity * $row['price']  // Total price for the item (quantity * price)
        ];
    }

    // Calculate total price (no delivery charge)
    $total_price = $order['total'];
    $order_date_time = $order['created_at'];  // Get order date and time
} else {
...
<img src="../images/logo.png" alt="Restaurant Logo" class="img-responsive">
    ...
    <div class="receipt-footer">
        ...
        <p><strong>Order Date & Time:</strong> <?php echo date("Y-m-d H:i:s", strtotime($order_date_time)); ?></p> <!-- Display order date and time -->
        <p><strong>Total Price:</strong> TK <?php echo $total_price; ?></p>  <!-- Total price of all items -->
    </div>
